    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const Contenido_ = await apiRest.fetchGet(`http://localhost:1337/api/contenidos?filters[id][$eq]=${contenido}`);
                console.log(Contenido_.data.data[0])
                if (Contenido_?.data?.data?.length > 0) {
                    setConteniido(Contenido_.data.data[0]);
                } else {
                    console.error("Error al obtener los datos:");
                }
                ...
                <div className='container-body'>
                    ...
                    <div className='banner-subjects'>
                        ...
                        <span className='banner-subjects-subtext ml-30 mt-05'>
                            {unidades?.nombre || 'Nombre de la unidad'} &gt; {conteniido?.titulo || 'Nombre del Contenido'}
                    ...
                    <div className='description-subjects-general mt-10 p-10'>
                        <p>
                            {conteniido?.descripcion?.length > 0 
                                ? conteniido.descripcion[0]?.children?.[0]?.text || "Sin descripción disponible." 
